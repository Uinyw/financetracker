...
public class LabelSetConverter implements AttributeConverter<Set<Label>, String> {
    ...
    public String convertToDatabaseColumn(Set<Label> labels) {
        return labels != null ? String.join(SPLIT_CHAR, labels.stream().map(Label::name).collect(Collectors.toSet())) : "";
    }

    @Override
    public Set<Label> convertToEntityAttribute(String string) {
        return string != null ? Arrays.stream((string.split(SPLIT_CHAR))).map(Label::new).collect(Collectors.toSet()) : Collections.emptySet();
    }
}
